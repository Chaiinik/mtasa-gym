...
function findRandomStartEndPoints()
    outputDebugString("findRandomStartEndPoints")
    repeat
        repeat
        ...
        until distance > 10 and distance < 300
    ...
    --setElementPosition(getAgentPlayer(), startNode.x, startNode.y, startNode.z)
    --createBlip(startNode.x, startNode.y, startNode.z, 0, 2, 255, 0, 0, 255, 0, 99999.0)
    --createBlip(endNode.x, endNode.y, endNode.z, 0, 2, 0, 255, 0, 255, 0, 99999.0)

    local rotX, rotY, rotZ = findRotation3D(path[1].x, path[1].y, path[1].z, path[2].x, path[2].y, path[2].z)
    --setElementRotation(getAgentPlayer(), 0, 0, rotZ)
    return Vector3(startNode.x, startNode.y, startNode.z), Vector3(endNode.x, endNode.y, endNode.z), Vector3(rotX, rotY, rotZ)
end

function Agent:constructor(i, start, destination, rotation)
    ...
    --self.vehicle = createVehicle(411, start, 0, 0, 180)
    ...
    triggerClientEvent(getAgentPlayer(), "onClientAgentCreated", resourceRoot, self.id, self.ped, self.vehicle, destination.x, destination.y, destination.z)
end
...
function startEnvironment(agent_id)
    ...
    --local start, finish, rot = findRandomStartEndPoints()
    --start = start + Vector3(0, 0, 2)
    --finish = finish + Vector3(0, 0, 2)
    --local start = Vector3(-1536.50354, 508.85898, 7.2)
    --local finish = Vector3(-2616.15674, 1390.70276, 7.11378)
    local start = Vector3(-1558.98877, 655.05096, 7.2)
    local finish = Vector3(-1640.72437, 1288.37024, 7.2)
    local rot = Vector3(0, 0, 0)
    local agent = new(Agent, agent_id, start, finish, rot)
